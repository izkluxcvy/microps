...
ether_tap_open(struct net_device *dev)
{
    ...
    if (val == -1) {
        errorf("fnctl(F_GETFL): %s, dev=%s", strerror(errno), dev->name);
        close(tap->fd);
        return -1;
    }
    if (fcntl(tap->fd, F_SETFL, val | O_ASYNC | O_NONBLOCK) == -1) {
        errorf("fnctl(F_SETFL): %s, dev=%s", strerror(errno), dev->name);
        close(tap->fd);
        return -1;
    }
    if (fcntl(tap->fd, F_SETSIG, tap->irq) == -1) {
        errorf("fnctl(F_SETSIG): %s, dev=%s", strerror(errno), dev->name);
        close(tap->fd);
        return -1;
    }
