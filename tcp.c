                    ...
                    ip_endp_t local, ip_endp_t remote)
{
        ...
        */
        if (TCP_FLG_ISSET(flags, TCP_FLG_SYN)) {
            ...
            if (pcb->mode == TCP_PCB_MODE_SOCKET) {
                ...
                debugf("allocate new PCB for new connection, desc=%d, state=%s",
    ...
    case TCP_STATE_TIME_WAIT:
        if (TCP_FLG_ISSET(flags, TCP_FLG_SYN)) {
            ...
            errorf("unexpected SYN, connection reset");
            TCP_STATE_CHANGE(pcb, TCP_STATE_CLOSED);
            tcp_pcb_release(pcb);
            return;
        }
...
tcp_input(const struct ip_hdr *iphdr, const uint8_t *data, size_t len, struct ip_iface *iface)
{
    ...
    seg.len = len - hlen;
    // SYN and FIN consume one sequence number even if they don't have payload
    ...
    if (active) {
        ...
        if (!local.port) {
            ...
            if (IP_ENDP_DYNAMIC_PORT_MAX < p) {
                errorf("failed to dynamic assign local port, addr=%s",
                       ip_addr_ntop(local.addr, addr, sizeof(addr)));
                lock_release(&lock);
                return -1;
            }
...
tcp_cmd_bind(int desc, ip_endp_t local)
{
    ...
    if (exist) {
        errorf("address already in use, addr=%s", ip_endp_ntop(local, ep, sizeof(ep)));
        lock_release(&lock);
        return -1;
    }
...
tcp_cmd_accept(int desc, ip_endp_t *remote)
{
    ...
    debugf("success, desc=%d, local=%s, remote=%s",
           new_desc,
